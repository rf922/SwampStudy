...
export class RatingController {
  ...
  public async postRating(req: Request, res: Response) {
      try {
        ...
        if( isNaN(parsedUserId) || isNaN(parsedRating)){//param check bad type
        ...
        const results = await this.ratingService.submitRating(parsedRating, parsedUserId);
  ...
  public async getRating(req: Request, res: Response) {
    try {
      ...
      if (isNaN(parsedUserId) && ! (parsedUserId < 1)) {// param chaeck
        return res.status(StatusCodes.BAD_REQUEST).json({ message: "Invalid Account ID" });
      }
      const avgRating = await this.ratingService.getRating(parsedUserId);
      if (avgRating === undefined) {
        return res.status(StatusCodes.NOT_FOUND).send({ message: "No rating found" });
      }
